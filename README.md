...
set listed on Polymarket. Resolution station and time window are read from each
Polymarket event's own metadata. In the current historical feed, NYC resolves to
LaGuardia (`KLGA`) and Chicago resolves to O'Hare (`KORD`) via Wunderground
metadata. The dashboard has city and date selectors. The Polymarket chart shows
every bracket price line for the selected event, and the model chart shows
generated model probabilities where available. Chart x-axes are fixed to the
selected event's resolution window; traces stop at their latest available
timestamp.
